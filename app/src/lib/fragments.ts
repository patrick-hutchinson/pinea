export const thumbnailFragment = `
  ...
  mediaType,
    ...
    "copyrightInternational": coalesce(image.copyrightInternational, video.copyrightInternational),
...
export const singleMediaFragment = `
{
  "type": select(
  ...
  "url": select(
    _type == "imageWithMetadata" => image.asset->url
  ),

  "lqip": select(
    _type == "imageWithMetadata" => image.asset->metadata.lqip
  ),

  "width": select(
    _type == "imageWithMetadata" => image.asset->metadata.dimensions.width
  ),
...
export const imageOrSlideshowFragment = `
  {
    ...
    "medium": select(
      _type == "media" => medium[0]${singleMediaFragment},

      // If it's a slideshow:
      _type == "slideshow" => {
        "gallery": gallery[] ${mediumQuery}
      }
    )
  }
`;
