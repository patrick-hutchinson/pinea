...
const sendEventSubmissionNotification = async ({ eventId, title, session }) => {
  ...
  if (!apiKey || !from) {
    console.warn(
      "[profile] Event submission notification skipped. Missing RESEND_API_KEY or PROFILE_EVENT_NOTIFICATION_FROM.",
    );
    return;
  }
...
export default async function handler(req, res) {
  ...
  if (!process.env.SANITY_MIGRATE_TOKEN) {
    fail(res, "Missing SANITY_MIGRATE_TOKEN. Draft creation is not configured.", 500);
    return;
  }
  ...
  if (!category || !title || !startDateInput || !endDateInput || !institution || !city || !countryId) {
    fail(res, "Missing required fields.");
    return;
  }

  if (uploadedImageAssetId && (!copyrightArtistName || !workTitle || !workYear)) {
    fail(res, "Artist, title of work and year of origin are required when an image is uploaded.");
    return;
  }
  ...
  if (!startDate || !endDate) {
    fail(res, "Start and end date must use DD.MM.YYYY.");
    return;
  }

  if (openingDateInput && !openingDate) {
    fail(res, "Opening date must use DD.MM.YYYY.");
    return;
  }

  if (openingTime && !/^([01]\d|2[0-3]):([0-5]\d)$/.test(openingTime)) {
    fail(res, "Opening time must use 24-hour format, e.g. 18:30.");
    return;
  }

  if (workYear && !/^\d{4}$/.test(workYear)) {
    fail(res, "Year of origin must be a 4-digit year.");
    return;
  }

  if (new Date(`${endDate}T00:00:00`).getTime() < new Date(`${startDate}T00:00:00`).getTime()) {
    fail(res, "End date cannot be before start date.");
    return;
  }
  ...
  try {
    ...
    if (!countryExists?._id) {
      fail(res, "Selected country does not exist.");
      return;
    }
    ...
    if (uploadedImageAssetId) {
      ...
      if (!imageAssetExists?._id) {
        fail(res, "Uploaded image asset was not found.");
        return;
      }
    }
    ...
    fail(res, error instanceof Error ? error.message : "Failed to create draft event.", 500);
  }
}
